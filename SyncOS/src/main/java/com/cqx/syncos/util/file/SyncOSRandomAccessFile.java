...
/**
 * SyncOSRandomAccessFile
 *
 * @author chenqixu
 */
public class SyncOSRandomAccessFile {
    private int index = 0;
    ...
    public SyncOSRandomAccessFile(String filename) throws FileNotFoundException {
        this.randomAccessFile = new RandomAccessFile(filename, MemoryCacheMode.READ_WRITE.getCode());
        this.fileChannel = this.randomAccessFile.getChannel();
        this.fileName = filename;
    }

    public SyncOSRandomAccessFile(String filename, boolean isGetData) throws IOException {
        this(filename);
        if (isGetData) {
            this.data = readAll();
        }
