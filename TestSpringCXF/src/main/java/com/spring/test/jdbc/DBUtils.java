...
//		return jdbcTemplateTT1.query(sql, new UserStatus());
        return query(sql, paramMap, UserStatus.class);
//        return namedParameterJdbcTemplateTT1.query(sql, paramMap, new UserStatus());
    ...
    public List<?> query(String sql, Map<String, ?> paramMap, Class<?> T) {
        try {
            return !ClassUtils.isPrimitiveWrapper(T) && !T.equals(String.class) ? this.namedParameterJdbcTemplateTT1.query(sql, paramMap, ParameterizedBeanPropertyRowMapper.newInstance(T)) : this.namedParameterJdbcTemplateTT1.queryForList(sql, paramMap, T);
...
//				+"from utap.auto_trigger_pr_conf where task_template_id=:id";
        ...
        sql = "select fn_11 as lacci,fs_11 as uptime from realtime_status_snapshot T1 where T1.fn_11 in (:lac_ci)";
