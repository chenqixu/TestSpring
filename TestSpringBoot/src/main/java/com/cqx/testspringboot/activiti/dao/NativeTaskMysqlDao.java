 ...
 */
@Repository
public class NativeTaskMysqlDao extends NativeTaskCommonDao {

    private static final Logger logger = LoggerFactory.getLogger(NativeTaskMysqlDao.class);
    ...
    public List<Task> queryUserTasks(ProcessQuery processQuery) {
        ...
        if (StringUtils.isNotEmpty(processQuery.getStartTime())) {
            sql.append(" and T.CREATE_TIME_ > DATE_FORMAT(#{startTime},'%Y-%m-%d %H:%i:%s')");
        }
        if (StringUtils.isNotEmpty(processQuery.getEndTime())) {
            sql.append(" and T.CREATE_TIME_ <= DATE_FORMAT(#{endTime},'%Y-%m-%d %H:%i:%s')");
        }
        ...
        logger.info("原生SQL mysql 任务查询:{},参数:{}", sql.toString(), processQuery.toString());
    ...
    public int queryUserTaskCount(ProcessQuery processQuery) {
        ...
        logger.info("原生SQL mysql 任务查询统计总数:{},参数:{}", sql.toString(), processQuery.toString());
