 ...
 */
@Repository
public class NativeHistoricProcessInstanceMysqlDao extends NativeHistoricProcessInstanceCommonDao {

    private static final Logger logger = LoggerFactory.getLogger(NativeHistoricProcessInstanceMysqlDao.class);
    ...
    public List<HistoricProcessInstance> queryHistoricInstances(ProcessQuery processQuery) {
        ...
        if (StringUtils.isNotEmpty(processQuery.getStartTime())) {
            sql.append(" and T.START_TIME_ > DATE_FORMAT(#{startTime},'%Y-%m-%d %H:%i:%s')");
        }
        if (StringUtils.isNotEmpty(processQuery.getEndTime())) {
            sql.append(" and T.START_TIME_ <= DATE_FORMAT(#{endTime},'%Y-%m-%d %H:%i:%s')");
        }
        ...
        logger.info("原生SQL mysql 历史查询:{},参数:{}", sql.toString(), processQuery.toString());
    ...
    public int queryHistoricInstanceCount(ProcessQuery processQuery) {
        ...
        logger.info("原生SQL mysql 历史查询统计总数:{},参数:{}", sql.toString(), processQuery.toString());
